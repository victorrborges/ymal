...
export default function BookmarksItem({
  ...
  openedItem: RecommendationObject | undefined;
  setOpenedItem: Function;
}) {
  const { authenticated, userId } = useAuthentication();
  const { recommendation, setRecommendation } = useRecommendation();
  const { bookmarks } = useBookmarks();
  const [expanded, setExpanded] = useState(openedItem?.url === item.url);
  ...
  useEffect(
    () => setExpanded(openedItem?.url === item.url),
    [openedItem]
  );
  ...
  return (
    <TouchableHighlight
      style={styles.item}
      onPress={() =>
        expanded ? setOpenedItem(undefined) : setOpenedItem(item)
      }
    >
      <>
        <motion.header
